...
import stevemerollis.codetrial.weather.fulfillment.model.FinishedIntent.Reason
import stevemerollis.codetrial.weather.host.HostActivity

sealed class Model<out T> {


    data class Success<out T>(val data: T): Model<T>() {
    ...
    companion object {
        ...
        fun from(result: AsyncResult.Error): Model<Nothing> {
            return when(result) {
